/**
 * Tool-output pruning for compaction.
 *
 * Walks session entries from newest to oldest, protects the most recent
 * `protectTokens` worth of tool results, then redacts older non-protected
 * tool results to a `[Output truncated - N tokens]` notice. Pruning runs
 * before threshold compaction so tool-heavy sessions can stay under the
 * threshold without invoking a model summarization pass.
 *
...
/**
 * Prune older tool-result messages in place. Mutates entry messages.
 * Returns 0/0 result if savings are below `minimumSavings` (no-op preserved).
 */
export function pruneToolOutputs(entries: SessionEntry[], config: PruneConfig = DEFAULT_PRUNE_CONFIG): PruneResult {
	...
	for (let i = entries.length - 1; i >= 0; i--) {
		...
		if ((message as any).prunedAt !== undefined) {
	...
	for (const candidate of candidates) {
		...
		(message as any).prunedAt = prunedAt;
		prunedCount++;
	}
