...
function registerSessionTool(pi: ExtensionAPI, state: SocketState): void {
	// @ts-expect-error TS2589: TypeBox version mismatch between local 0.34 and pi-mono 1.x causes deep type instantiation
	pi.registerTool({
...
Messages automatically include sender session info for replies. When you want a response, instruct the target session to reply directly to the sender by calling send_to_session with the sender_info reference (do not poll get_message).`,
		...
		renderResult(result, { expanded }, theme) {
			...
			// Error case
			if (details?.error) {
				const errorMsg = (details?.error as string) || (Array.isArray(result.content) && result.content[0]?.type === "text" ? (result.content[0] as { type: "text"; text: string }).text : "Unknown error");
