...
// sliceByColumn is not exported from pi-tui, so we implement a simple version
function sliceByColumn(line: string, startCol: number, length: number, _strict = false): string {
	// Simple implementation: truncate from start, then to length
	const chars = [...line];
	const sliced = chars.slice(startCol, startCol + length);
	return sliced.join("");
}
